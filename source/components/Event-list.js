...
* Jason Boenjamin
*
Database Functionality added by the Entire Backend Team
*/
export default class EventListItem extends HTMLElement{
  ...
  constructor(){
      ...
      /*
      * Create a modal that will create new events
      */
  ...
  */ 
  ...
  editEvent(newListElement){
    newListElement.addEventListener('dblclick', () => {            
      ...
      /*create a new Div that becomes the edit Modal
      *Then grab all the information from the event entry 
      *and load it into the edit Modal 
      */
      ...
      `;
      //Find all 3 edit Modal buttons and add their functionality
  ...
  addNewEvent(object = undefined){
    ...
    if(!object){
      ...
      eventTime.setHours(object['time_end'].substring(0,2));
      eventTime.setMinutes(object['time_end'].substring(3,5));
      if(eventTime>curDate){
        newListElement.innerHTML += `
        ...
        newListElement.innerHTML += `
        <div id="event-entry" class="${object["tag"]}-passed" tag="${object["tag"]}" time_start= "${object["time_start"]}" time_end ="${object["time_end"]}" title = "${object["title"]}" info = "${object["info"]}">
        <div id="entry-title">${object["title"]}</div>
        <div id="time">${object["time_start"]}-${object["time_end"]}</div>
        </div> 
        `;
   ...
   * @returns {void}
   * @summary Clears the List so it can be repopulated
   */
